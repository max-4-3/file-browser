# file-browser (my local video gallery) 🎬
...
- Config: open `src/__init__.py` to change the shared folder path and a few basic settings. This is where you can point the gallery to the folder you want to serve.
- Backend behavior:
...
3. Change the folder that is served:
   - Edit `src/__init__.py` and tweak the shared folder path / basic settings. It's a personal project so feel free to hack it.
